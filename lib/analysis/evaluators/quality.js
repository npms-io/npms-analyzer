...
/**
 * Evaluates the module dependencies.
 * Takes into consideration the outdated dependencies as well as vulnerable dependencies.
 *
 * @param {object} info The info object returned from the collectors
 *
 * @return {number} The tests evaluation (from -Infinity to 0)
 */
function evaluateDependencies(info) {
    ...
    const improvementCount = outdatedCount * 0.6 +
                             vulnerabilitiesCount * 0.4;

    return improvementCount ? -improvementCount : 0;
}
...
function evaluateBranding(info) {
    ...
    return Number(hasCustomWebsite) * 0.5 +
           badgesCount * 0.5;
}
...
function quality(info) {
    return {
        ...
        dependencies: evaluateDependencies(info),
