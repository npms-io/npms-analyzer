...
class Queue extends EventEmitter {
    ...
    push(data) {
        ...
        log.silly(logPrefix, 'Pushing into the queue', { data });

        // Ensure we are connected
        return this._connect()
        // Add it to the queue
        .then(() => {
            return this._sendToQueue({ data, pushedAt: (new Date()).toISOString(), retryCount: 0 });
        })
        .then(() => {
            log.silly(logPrefix, 'Successfully pushed into the queue', { data });
        }, (err) => {
            log.error(logPrefix, 'Failed to push data into the queue', { data, err });
            throw err;
        });
    }
    ...
    _connect() {
        ...
        .then(() => {
            ...
            .once('blocked', (reason) => {
                log.error(logPrefix, `RabbitMQ is blocking the connection, reason being ${reason}`, { reason });
                this.destroy();
            ...
            .once('error', (err) => this._reconnect(err))
            .once('close', (err) => this._reconnect(err))
            .once('return', () => log.error(logPrefix, 'A message could not be queued and was returned back'));

            log.info(logPrefix, 'RabbitMQ connect and setup done successfully');
        })
    ...
    _reconnect(err) {
        ...
        if (this._destroyed) {
            log.silly(logPrefix, 'Reconnect ignored because instance is already destroyed');
            return;
        }

        // Do not reconnect if already reconnecting..
        if (this._reconnectDelayPromise) {
            log.error(logPrefix, 'RabbitMQ connection errored, already retrying to reconnect to..',
                { err, attemptCount: this._reconnectCount });
            return;
        }

        log.error(logPrefix, 'RabbitMQ connection errored, reconnecting in a few moments..', { err, attemptCount: this._reconnectCount });
    ...
    _registerConsumer(consumer) {
        ...
        return this._channel.consume(this._name, (queueMsg) => {
            ...
            if (!queueMsg) {
                log.error(logPrefix, 'Consumer function got canceled remotely..');
                this.destroy();
        ...
        .then(() => {
            log.info(logPrefix, 'Consumer was registered successfully, will now receive messages..');
        }, (err) => {
            this._consumer = null;
            log.error(logPrefix, 'Failed to register consumer', { err });
            throw err;
        });
    }
    ...
    _handleConsumerSuccess(msg, queueMsg) {
        ...
        if (!this._isConnected()) {
            log.warn(logPrefix, 'Consumer handled the message but we are no longer connected', { msg });
            return;
        }

        log.verbose(logPrefix, 'Consumer successfully handled message, ack\'ing..', { msg });
    ...
    _handleConsumerError(err, msg, queueMsg) {
        // Handle the fact that we might no longer be connected.. consumers might take a while to handle the message
        if (!this._isConnected()) {
            log.warn(logPrefix, 'Consumer failed to handle the message but we are no longer connected', { msg });
            return;
        }

        // Did we reach the max allowed retries?
        if (msg.retryCount >= this._consumer.maxRetries) {
            log.error('queue', `Failed to consume message, NOT re-queueing after ${msg.retryCount} failed attempts`, { err, msg });
            this._channel.nack(queueMsg, false, false);
            return;
        }

        log.info(logPrefix, 'Failed to consume message, re-queueing..', { err, msg });
        msg.retryCount += 1;
        ...
        .catch((err) => log.error(logPrefix, 'Error trying to re-queue the message, duplicates will occur!', { err }))
        .done();
    }
