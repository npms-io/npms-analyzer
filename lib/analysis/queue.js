...
class Queue extends EventEmitter {
    ...
    _reconnect(err) {
        ...
        log.error(logPrefix, 'RabbitMQ connection errored, reconnecting in a few moments..',
            { err, attemptCount: this._reconnectCount });
    ...
    _handleConsumerError(err, msg, queueMsg) {
        ...
        if (msg.retryCount >= this._consumer.maxRetries) {
            log.error('queue',
                `Failed to consume message, NOT re-queueing after ${msg.retryCount} failed attempts`, { err, msg });
            this._channel.nack(queueMsg, false, false);
            return;
        }

        log.error(logPrefix, 'Failed to consume message, re-queueing..', { err, msg });
        msg.retryCount += 1;
        msg.pushedAt = (new Date()).toISOString();
