    ...
    return promiseRetry((retry) => {
        ...
        return tokenDealer(options.tokens, (token, exhaust) => {
            ...
            wait: (token, duration) => {
                ...
                log.stat(logPrefix, `All tokens are exhausted, next one will become available in ${duration}s`);
                return true;
            },
        })
        .then((response) => {
    ...
    return promiseRetry((retry) => {
        ...
        .then((response) => {
            ...
            try {
                ...
                // If the page still seems invalid, try again.. it usually works next time
...
function github(data, packageJson, options) {
    ...
    .then((data) => {
        ...
        if (missingData.length) {
            log.warn(logPrefix, 'Can\'t analyze because there\'s missing data', { missingData });
            return null;
        }
