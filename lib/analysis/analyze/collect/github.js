...
function githubRequest(resource, options) {
    ...
    return promiseRetry((retry) => {
        ...
        .then((response) => {
            ...
            if (response.statusCode === 202) {
                ...
                retry(Object.assign(new Error(`Empty response for ${url}`), { code: 'EMPTY_RESPONSE' }));
            }
            ...
            if (err.statusCode === 404 || err.statusCode === 400 || err.statusCode === 403) {
                log.warn(logPrefix, `GitHub request to ${url} failed with ${err.statusCode}`, { err });
                return null;
            }
    ...
    .catch((err) => {
        // If after all the retries there's still no content, return the empty response..
...
function fetchIssuesStats(repository, options) {
    ...
    .then((stats) => {
        ...
        // 404 - not found; 400 - invalid repo name, 403 - dmca takedown
        if (err.statusCode === 404 || err.statusCode === 400 || err.statusCode === 403) {
            log.warn(logPrefix, `Fetch of issues stats for ${repository} failed with 404`, { err });
            return null;
        }
    ...
    .then((data) => {
        ...
        if (missingData.length) {
            if (missingData.length === dataKeys.length) {
                log.warn(logPrefix, `The GitHub repository ${repository} no longer exists`);
            } else {
                log.warn(logPrefix, `It seems that the GitHub repository ${repository} is empty`);
            }
