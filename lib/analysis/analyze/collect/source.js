...
function checkDepsVulnerabilities(name, dir) {
    ...
    .catch((err) => {
        // Some old packages have invalid shrinkwrap files.. so we ignore these
        if (/"statusCode":400.+shrinkwrap\.name/.test(err.stderr)) {
            log.warn(logPrefix, `Ignoring broken shrinkwrap file while scanning ${name} dependencies with nsp`,
                { err, dir });
            return [];
        }
