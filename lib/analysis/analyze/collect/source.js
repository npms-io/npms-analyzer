...
function checkDepsVulnerabilities(name, dir) {
    ...
    .catch((err) => {
        ...
        // e.g.: invalid shrinkwrap files, etc
        if (/"statusCode":400/i.test(err.stderr)) {
            log.warn(logPrefix, `Ignoring bad request error while scanning ${name} dependencies with nsp`,
                { err, dir });
            return [];
        }
...
function checkOutdatedDeps(data, packageJson, options) {
    ...
    .then((deps) => {
        ...
        if (/failed to get versions/i.test(err.message)) {
            log.warn(logPrefix, `Some of ${data.name} dependencies are broken, skipping check outdated..`, { err });
            return {};
        }

        log.error(logPrefix, `Failed to check outdated dependencies of ${data.name}`, { err });
        throw err;
    });
}
