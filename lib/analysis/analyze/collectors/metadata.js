...
function normalizeLicense(name, license) {
    ...
    if (typeof license !== 'string' || !license) {
        log.silly('metadata', `Invalid license for module ${name} was found`, { license });
        return null;
    }
    ...
    if (!spdx.valid(license)) {
        ...
        if (correctedLicense) {
            log.verbose('metadata', `Module ${name} license was corrected from ${license} to ${correctedLicense}`);
            license = correctedLicense;
        } else {
            log.verbose('metadata', `License for module ${name} is not a valid SPDX indentifier`, { license });
            license = null;
        }
    }
...
function metadata(data, packageJson) {
    return Promise.try(() => {
        ...
        return deepCompact({
            ...
            releases: {
                latest: { version: versions[versions.length - 1], date: get(data, 'time.modified') },
                first: { version: versions[0], date: get(data, 'time.created') },
