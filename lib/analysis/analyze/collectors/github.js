...
function githubRequest(resource, options) {
    ...
    return promiseRetry((retry) => {
        // Use token dealer to use configured API keys!
        return tokenDealer(options.tokens, (token, exhaust) => {
            const handleResponse = (response, err) => {
                if (response.headers['x-ratelimit-remaining'] === '0') {
                    log.warn('github', 'Token ${token.substr(0,6)}.. exhausted');
            ...
            wait: (token, duration) => {
                ...
                log.warn('github', `All tokens are exhausted, next one will become available in ${duration}s`);
                return true;
            },
        })
        .then((response) => {
