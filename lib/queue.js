...
class Queue extends EventEmitter {
    ...
    push(data) {
        ...
        // Add it to the queue
        .then(() => {
            return this._sendToQueue({ data, persistedAt: Date.now(), retryCount: 0 });
        })
        .then(() => {
    ...
    _reconnect(err) {
        ...
        // Emit the connect event and check if the listener have decided to destroy the instance
    ...
    _registerConsumer(consumer) {
        ...
        return this._channel.consume(this._name, (queueMsg) => {
            ...
            // cancels this consumer, queueMsg will be null
            if (!queueMsg) {
