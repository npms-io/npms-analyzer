...
class RealtimeObserver {
     ...
     */
    _fetchLastSeq() {
        return this._npmsNano.getAsync('realtime!last_followed_seq')
        .then((doc) => {
            this._lastSeq = doc;
        })
        .catch({ error: 'not_found' }, () => {
            this._lastSeq = {
                _id: 'realtime!last_followed_seq',
