...
class RealtimeObserver {
    ...
    _start() {
        ...
        .catch(this._ignoreIfStopped((err) => {
            log.error(logPrefix, 'Realtime observer failed, restarting in a few moments..', { err });
    ...
    _updateLastSeq(seq) {
        ...
        .then(this._ignoreIfStopped((inserted) => {
            log.verbose(logPrefix, `Last followed seq updated to ${seq}`, { seq });

            this._lastSeq.value = seq;
            this._lastSeq._rev = inserted.rev;
        }))
        .catch({ error: 'conflict' }, this._ignoreIfStopped(() => {
            log.warn(logPrefix, 'Are two realtime instances running simultaneously?');
    ...
    _flushBuffer() {
        ...
        .spread(this._ignoreIfStopped((filteredNames, bulkUpdate) => {
            ...
            filteredNames && log.verbose(logPrefix, `Notifying ${filteredNames.length} changed modules`);
            ignoredCount && log.warn(logPrefix, `Ignored ${ignoredCount} modules because they have the same version`);
        ...
        .then(this._ignoreIfStopped(() => {
            ...
            .catch((err) => log.error(logPrefix, 'Failed to update last followed seq, ignoring..', { seq, err }));
        }))
        ...
        .then(this._ignoreIfStopped(() => this._follower.resume()), this._ignoreIfStopped((err) => {
            log.error(logPrefix, 'Realtime failed when notifying modules, restarting in a few moments..', { err });
     ...
     * Though, we are only interested in changes that were produced by a publish.
     *
    ...
    _filterNewVersions(names) {
        ...
        .spread(this._ignoreIfStopped((npmRows, observerRows) => {
            // Filter only the ones that have their version changed or removed,
            // aggregating the docs and patches for each one
            const docsHash = {};
            const patchesHash = {};

            const filteredNames = names.filter((name, index) => {
                const npmVersion = npmRows[index].doc ? get(npmRows[index].doc, 'dist-tags.latest') || '0.0.1' : null;
                const observerVersion = get(observerRows[index], 'doc.realtime.version');
                const hasNewVersion = !npmVersion || !observerVersion || observerVersion !== npmVersion;
                ...
                patchesHash[name] = { _id: `observer!module!${name}`, realtime: { version: npmVersion } };
