...
class RealtimeObserver {
     ...
     * Constructor.
     *
     * Note that the `onModules` function may return a promise that will waited
     * before resuming the observe process.
     *
     * @param {string}   npmAddr   The npm address (connection string)
     * @param {string}   npmsAddr  The npms address (connection string)
     ...
     */
    constructor(npmAddr, npmsAddr, onModules, options) {
        ...
        this._options = merge({
            ...
            requestDefaults: { timeout: 5000 },  // Request defaults to be used by nano
        }, options);

        // Setup couchdb's nano instances
        this._npmCouchdb = Promise.promisifyAll(nano(npmAddr, { requestDefaults: this._options.requestDefaults }));
        this._npmsCouchdb = Promise.promisifyAll(nano(npmsAddr, { requestDefaults: this._options.requestDefaults }));
    ...
    _start() {
        ...
        log.info('realtime', 'Starting realtime watch..');
        ...
        .catch(this._ignoreIfStopped((err) => {
            log.error('realtime', 'Realtime failed, restarting in a few moments..', { err });
     ...
     */
    _fetchLastSeq() {
        return this._npmsCouchdb.getAsync('last_followed_seq')
        .then((doc) => {
            this._lastSeq = doc;
        }, (err) => {
    ...
    _followChanges() {
        ...
        log.info('realtime', 'Will now start to follow changes since ' + since, { since });

        this._buffer = [];

        this._follower = this._npmCouchdb.follow({ since });
        this._follower
        // Buffer each change
     ...
     */
    _flushBuffer() {
        log.verbose('realtime', 'Flushing changes buffer with a total of ' + this._buffer.length + ' changes..');
        ...
        .then(this._ignoreIfStopped((inserted) => {
            log.verbose('realtime', 'Flushed done, last followed seq updated to ' + seq, { seq });

            this._lastSeq._rev = inserted.rev;
            this._lastSeq.value = seq;
        }), this._ignoreIfStopped((err) => {
            log.error('realtime', 'Unable to update last followed seq', { seq, err });
            err.error === 'conflict' && log.warn('realtime', 'Are two realtime instances running simultaneously?');
