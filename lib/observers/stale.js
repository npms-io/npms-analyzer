...
const logPrefix = 'observer/stale';
...
class StaleObserver {
    ...
    _start() {
        ...
        log.info(logPrefix, 'Starting stale observer..');
     ...
     */
    _check() {
        log.verbose(logPrefix, 'Looking for stale modules..');
        ...
        couchdbIterator.bulk(this._npmsNano, 'npms-analyzer/modules-stale', this._ignoreIfStopped((rows) => {
            ...
            log.verbose(logPrefix, `Got ${rows.length} modules that are considered stale, filtering..`);
            ...
            .spread(this._ignoreIfStopped((filteredNames, bulkUpdate) => {
                ...
                filteredNames && log.verbose(logPrefix, `Notifying ${filteredNames.length} stale modules`);
                ignoredCount && log.info(logPrefix, `Ignored ${ignoredCount} modules because they were already notified`);
        ...
        .catch(this._ignoreIfStopped((err) => {
            log.error(logPrefix, 'Stale observer failed when notifying modules', { err });
        }))
