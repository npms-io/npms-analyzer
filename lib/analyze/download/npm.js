...
function download(target, url, tmpDir, options) {
    ...
    let request;
    let requestStream = got.stream(url, { timeout: 15000, retries: gotRetries })
    ...
    return pump(
        ...
        .on('response', (response) => {
            ...
            if (contentLength > options.maxSize) {
                request.abort();
                requestStream.emit('error', Object.assign(new Error(`Tarball is too large (~${Math.round(contentLength / 1024 / 1024)}MB)`), {
                    unrecoverable: true,
                }));
            }
        }),
        fs.createWriteStream(tarballFile)
    )
    // Extract tarball
    .then(() => {
