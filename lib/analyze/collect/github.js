...
function github(data, packageJson, options) {
    ...
    options = Object.assign({
        ...
        ref: packageJson.gitHead || 'master',  // The commit ref to use to retrieve statuses, defaults to packageJson.gitHead or master
    }, options);
    ...
    return promisePropsSettled({
        ...
        statuses: githubRequest(`/repos/${repository}/commits/${options.ref}/statuses`, options),
    ...
    .then((data) => {
        ...
        if (missingData.length) {
            if (missingData.length === dataKeys.length) {
                log.info(`The GitHub repository ${repository} no longer exists, is invalid or blocked`);
            } else {
