...
const snykBin = path.normalize(`${__dirname}/../../../node_modules/.bin/snyk`);
 ...
 */
function checkVulnerabilities(packageJson, dir, options) {
    if (!options.snykToken) {
        return Promise.reject(new Error('Unable to check package vulnerabilities because of missing snyk token'));
    }

    const jsonFile = `${dir}/.npms-snyk.json`;
    let error;
    ...
    return exec(exec.escape`${snykBin} test ${packageJson.name}@${packageJson.version} --json > ${jsonFile}`, {
        timeout: 60 * 1000,
        env: Object.assign({}, process.env, { SNYK_TOKEN: options.snykToken }),
    })
    // The snyk CLI exists with errorcode if there's vulnerabilities
    // Catch the error here and deal with it in the final catch handler
    .catch((err) => { error = err; })
    .then(() => loadJsonFile(jsonFile))
    .then((output) => output.vulnerabilities)
    .map((vulnerability) => ({
        title: vulnerability.title,
        description: vulnerability.description,
        createdAt: vulnerability.creationTime,
        updatedAt: vulnerability.modificationTime,
        publishedAt: vulnerability.publicationTime,
        severity: vulnerability.severity,
        cvssScore: vulnerability.cvssScore,
        package: vulnerability.packageName,
        path: vulnerability.from,
    }))
    .catch((err) => {
        err = error || err;
        /* istanbul ignore next */
...
function source(data, packageJson, downloaded, options) {
    options = Object.assign({
        ...
        snykToken: null,  // The snyk API token to use (mandatory)
    }, options);
    ...
    .then((props) => {
        return promisePropsSettled({
            ...
            vulnerabilities: checkVulnerabilities(packageJson, downloaded.packageDir, options),
        })
