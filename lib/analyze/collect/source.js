...
function checkVulnerabilities(packageJson, dir, options) {
    ...
    const url = `https://snyk.io/api/v1/vuln/npm/${encodeURIComponent(target)}`;

    return got(url, {
        json: true,
        headers: {
            Authorization: `token ${options.snykToken}`,
        },
    })
    .then((res) => res.body.vulnerabilities)
    .map((vulnerability) => ({
    ...
    .catch({ statusCode: 404 }, () => {
        log.warn({ url }, `The Snyk API returned 404 when fetching ${target} vulnerabilities`);

        return [];
    })
    .catch((err) => {
        /* istanbul ignore next */
        log.error({ err }, `Failed to scan ${target} vulnerabilities`);
        /* istanbul ignore next */
        throw err;
    });
}
