...
function inspectFiles(data, dir) {
    ...
    const testsSize = glob('**/*(_){test,spec}?(s)*(_)/', {
        cwd: dir,
        ignore: 'node_modules/**',
        silent: true,   // Do not crash on the first error
        strict: false,  // Do not print warnings
    })
    ...
    return Promise.props({
        ...
        hasShrinkwrap: isFile(`${dir}/npm-shrinkwrap.json`),
    });
}
...
function checkDepsVulnerabilities(name, dir) {
    ...
    return promiseRetry((retry) => {
        ...
        .catch((err) => /"statusCode"\s*:\s*503/.test(err.stderr), (err) => {
            log.warn(logPrefix, 'The nsp service appears to be unavailable (503), retrying..', { err });
            retry(err);
        })
        ...
        .catch((err) => err.stderr && err.stderr.indexOf('53,48,52,32,71,97,116,101,119,97,121,32,84,105,109,101,45,111,117,116') !== -1, (err) => {  // eslint-disable-line max-len
            log.warn(logPrefix, 'The nsp service appears to be unavailable (504), retrying..', { err });
            retry(err);
        })
    ...
    .catch((err) => err.stderr === 'Debug output: undefined\n{}\n', (err) => {
        log.warn(logPrefix, `Ignoring unknown error while scanning ${name} dependencies with nsp`, { err, dir });
        return false;
    })
    .catch((err) => {
...
function source(data, packageJson, downloaded, options) {
    ...
    return promisePropsSettled({
        ...
        repositorySize: fileSize(downloaded.dir),
        ...
        linters: scanRepoLinters(packageJson.name, downloaded.dir),
        coverage: fetchCoverageUsingBadges(badges),
        badges,
    })
    .then((result) => deepCompact(result))
    .tap(() => log.verbose(logPrefix, `The source collector for ${packageJson.name} completed successfully`));
}
