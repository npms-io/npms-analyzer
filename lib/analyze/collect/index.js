...
/**
 * Checks if a module matches the downloaded repository.
 *
...
function checkRepositoryOwnership(data, packageJson, downloaded, npmNano) {
    ...
    // Check if the repo belongs to the same user/org
    const gitInfo = hostedGitInfo(repositoryUrl);
    const downloadedGitInfo = hostedGitInfo(downloadedRepositoryUrl);

    if (gitInfo && downloadedGitInfo && gitInfo.type === downloadedGitInfo.type && gitInfo.user === downloadedGitInfo.user) {
        return Promise.resolve(true);
    }
    ...
    // Do a final check against the maintainers of the downloaded module
