...
function extractLinks(packageJson) {
    ...
    const areLinksWorkingCache = {};
    const areLinksWorking = mapValues(links, (link, name) => {
        // Note that we avoid checking the npm link because we are sure it works..
        if (name === 'npm') {
            return true;
        }
        ...
        return areLinksWorkingCache[normalizedLink] = areLinksWorkingCache[normalizedLink] || isLinkWorking(normalizedLink);
    });

    log.debug({ links: Object.keys(areLinksWorkingCache) }, 'Checking broken links');

    return Promise.props(areLinksWorking)
    .then((result) => Object.keys(pickBy(result)))
    .then((names) => pick(links, names))
    .tap((goodLinks) => {
        const brokenLinksNames = difference(Object.keys(links), Object.keys(goodLinks));

        if (brokenLinksNames.length) {
            log.info({ brokenLinks: pick(links, brokenLinksNames) }, `Removed ${brokenLinksNames.length} broken links`);
        }
    });
}
