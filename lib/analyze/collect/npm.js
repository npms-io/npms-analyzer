...
function fetchDownloads(name) {
    ...
    // Their API do not respect the HTTP status codes.. they respond 200 OK even if the request failed..
    .then((response) => {
...
function fetchDependentsCount(name, npmNano) {
    ...
    .then((res) => !res.rows.length ? 0 : res.rows[0].value)
    .catch((err) => {
...
function npm(data, packageJson, npmNano) {
    ...
    .tap(() => log.verbose(logPrefix, 'The npm collector completed successfully'));
}
