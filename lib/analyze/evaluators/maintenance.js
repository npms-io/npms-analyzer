 ...
 */
function calculateRecentCommitsScore(info) {
 ...
 * @return {number} The score
 */
function calculateCommitsFrequencyScore(info) {
    const commits = info.github && info.github.commits;
 ...
 * @return {number} The score
 */
function calculateIssuesCountScore(info) {
    ...
    const openIssuesPercentage = issues.count ? issues.openCount / issues.count : 0;
 ...
 * @return {number} The score
 */
function calculateIssuesDistributionScore(info) {
    const issues = info.github && info.github.issues;
...
function maintenance(info) {
    const scores = {
        recentCommits: calculateRecentCommitsScore(info),
        commitsFrequency: calculateCommitsFrequencyScore(info),
        issuesCount: calculateIssuesCountScore(info),
        issuesDistribution: calculateIssuesDistributionScore(info),
    };
