...
function normalizePackageJson(packageJson) {
    ...
    if (!packageJson.version) {
        log.warn(logPrefix, `No version for ${packageJson.name}, mocking version in package.json..`);
    ...
    try {
        ...
        log.warn(logPrefix, `Error while normalizing ${packageJson.name} package.json, mocking it..`, { err });
