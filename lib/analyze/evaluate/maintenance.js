...
/**
 * Checks if a module is finished, that is, it's stable enough that doesn't require a lot of maintenance.
 *
 * @param {object} collected The collected information
 *
 * @return {boolean} True if finished, false otherwise
 */
function isModuleFinished(collected) {
    ...
    log.debug({ isStable, isNotDeprecated, hasFewIssues, hasREADME, hasTests },
        `Module is considered ${isFinished ? 'finished' : 'unfinished'}`);
...
function maintenance(collected) {
    ...
    // If the module is finished, it doesn't require a lot of maintenance
    if (isModuleFinished(collected)) {
