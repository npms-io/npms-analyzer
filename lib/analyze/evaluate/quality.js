...
function evaluateHealth(collected) {
    ...
    // Note that if the package is has npm-shrinkwrap.json, then it actually has its versions locked down
 ...
 * Takes into consideration if the package has a custom homepage and its badges.
 *
 * @param {object} collected The collected information
 *
...
/**
 * Evaluates the package self-admitted technical debts.
 *
 * @param {object} collected The collected information
 *
 * @return {number} The technical debt evaluation (from 0 to 1)
 */
function evaluateTechnicalDebts(collected) {
    if (!collected.source) {
        return 0;
    }

    return 1 - collected.source.technicalDebts;
}
...
function quality(collected) {
    return {
        ...
        technicalDebts: evaluateTechnicalDebts(collected),
    };
}
