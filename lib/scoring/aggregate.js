...
function remove(npmsNano) {
    ...
    .then(() => log.verbose(logPrefix, 'Aggregation removed'));
}
...
function aggregate(npmsNano) {
    ...
    log.silly(logPrefix, 'Starting aggregation..');
    ...
    return couchdbIterator(npmsNano, 'npms-analyzer/modules-evaluation', (row, index) => {
        ...
        !index && shapeAggregation(aggregation, row.value);  // Shape aggregation on the first evaluation
        aggregateEvaluation(aggregation, index, row.value);  // Aggregate this evaluation into the aggregation object
    ...
    // We are done!
    .then((aggregation) => {
        log.verbose(logPrefix, `Aggregation successful, processed a total of ${aggregation.count} evaluations`, { aggregation });
        return aggregation;
    }, (err) => {
