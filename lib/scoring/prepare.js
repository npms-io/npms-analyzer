...
const logPrefix = 'scoring/prepare';
...
function prepare(esClient) {
    ...
    log.info(logPrefix, 'Preparing scoring..');
    ...
    return Promise.try(() => {
        ...
        .then(() => log.verbose(logPrefix, 'Gathered elasticsearch info..', esInfo));
    })
    ...
    .then(() => {
        ...
        .then(() => log.verbose(logPrefix, 'Created new index', { index: esInfo.newIndex }));
    })
    ...
    .then(() => {
        ...
        .then(() => log.verbose(logPrefix, 'Updated npms-write alias', { actions }));
    })
    ...
    .then(() => {
        ...
        .then(() => log.verbose(logPrefix, 'Removed unnecessary indices', { indices }));
    })
    .return(esInfo);
}
