 ...
 */
function cubeRoot(x) {
    return new Big(Math.pow(x.abs().valueOf(), 1 / 3)).times((x.lt(0) ? -1 : 1));
}
...
function solveCubic(a, b, c, d) {
    ...
    if (p.abs().lt(0)) { // p = 0 -> t^3 = -q -> t = -q^1/3
        ...
        if (D.abs().lt(0)) {       // D = 0 -> two roots
            ...
            const t = new Big(Math.acos(q.times(3).div(p).div(u).valueOf())).div(3); // D < 0 implies p < 0 and acos argument in [-1..1]
            const k = new Big(2).times(Math.PI).div(3);
