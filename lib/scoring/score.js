...
function buildScore(analysis, aggregation) {
    ...
    return {
        ...
        flags: compact([
            collected.metadata.deprecated && 'deprecated',
            semver.lt(collected.metadata.version, '1.0.0', true) && 'unstable',  // `true` = loose semver
            (collected.source && collected.source.dependenciesVulnerabilities || []).length > 0 && 'insecure',
        ]),
        evaluation,
        score: {
...
function getLivingIndices(esClient) {
    log.trace('Getting living indices score');
...
function storeScore(score, livingIndices, esClient) {
    ...
    log.trace({ score, livingIndices }, `Storing score for ${name}`);
...
function all(aggregation, npmsNano, esClient) {
    ...
    .then((livingIndices) => {
        log.info({ livingIndices, aggregation }, 'Scoring packages..');
