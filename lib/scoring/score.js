...
function calculateScore(value, aggregation) {
    ...
    const roots = solveCubic(new Big(1), new Big(normAvg).times(-3), new Big(normAvg).times(3), new Big(normValue).times(-1));

    let t = null;

    roots.some((value) => {
        if (value.gte(0) && value.lte(1)) {
            t = value;
            return false;
        }
    ...
    return Number(t.pow(3).minus(new Big(3).times(avgY).times(t.pow(2))).add(new Big(3).times(avgY).times(t)).valueOf());
}
