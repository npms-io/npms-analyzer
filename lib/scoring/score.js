...
function calculateScore(value, aggregation, avgY) {
    ...
    if (t === null) {
        ...
        throw Object.assign(new Error('Solving the Cubic formula failed, probably aggregation is incorrect.'),
            { roots, normValue, normAvg, value, aggregation });
    }
...
function all(aggregation, npmsNano, esClient) {
    ...
    .then((livingIndices) => {
        livingIndices = livingIndices.indexOf('npms-new') !== -1 ? ['npms-new'] : [];

        if (!livingIndices.length) {
            return Promise.reject(Object.assign(new Error('There is no `npms-new` scoring index.'), { code: 'SCORE_INDEX_NOT_FOUND' }));
        }
