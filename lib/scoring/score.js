...
const Bezier = require('gm-bezier').Bezier;
const couchdbIterator = require('couchdb-iterator');
const aggregate = require('./aggregate');
const weightedMedian = require('./util/weightedMedian');
const quadratic = require('quadratic').quadraticSolve;
const avgY = 0.75; // [0, 1]
const curvePrecision = 6;
...
function calculateScore(value, aggregation) {
    ...
    return aggregation.curve.findYAtX(normValue);
}
...
function all(aggregation, npmsNano, esClient) {
    ...
    log.silly(logPrefix, 'Calculating aggregation curves..');

    aggregation = calculateAggregationCurves(aggregation);

    log.silly(logPrefix, 'Iterating modules..');
...
function score(analysis, npmsNano, esClient) {
    ...
    .then(calculateAggregationCurves)
    .then((score) => {
...
function calculateCurve(aggregation) {
    ...
    return new Bezier()
        ...
        .c3([normAvg, avgY])
        .c4([1, 1])
        .buildLookup(Math.pow(10, curvePrecision));
}


function calculateAggregationCurves(agreggation) {
    deepForEach(agreggation, (value) => {
        if (value === Object(value)
            && value.hasOwnProperty('min')
            && value.hasOwnProperty('max')
            && value.hasOwnProperty('mean')
        ) {
            value.curve = calculateCurve(value);
        }
    });
