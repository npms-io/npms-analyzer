...
function calculateScore(value, aggregation) {
    ...
    const roots = solveCubic(Big(1), Big(normAvg).times(-3), Big(normAvg).times(3), Big(normValue).times(-1));
    ...
    return Number(t.pow(3).minus(Big(3).times(avgY).times(t.pow(2))).add(Big(3).times(avgY).times(t)).valueOf());
}
